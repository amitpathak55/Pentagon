...
def upload_student_excel_data(request):
    path_to_excel = os.path.join(settings.BASE_DIR, 'data') + '/student-data.xlsx'
    ...
    columns = (
        ...
        'immigration',
        'bachelor_major',
        'bachelor_major_country',
        'master_major',
        'master_major_country',
        'note'
    )
    for row in ws.iter_rows(min_row=2):
        ...
        for index, cell in enumerate(row):
            try:
                ...
                return JsonResponse({'upload_status': 'Error encountered while parsing excel.'})
        try:
            with transaction.atomic():
                student_data = dict(row_data)
                keys_to_remove = [
                    'bachelor_major',
                    'bachelor_major_country',
                    'master_major',
                ...
                for key in keys_to_remove:
                    student_data.pop(key)
                student = Student(**student_data)
                student.save()
                if(Student.objects.filter(student_id=row_data['student_id']).count() == 0):
                    if row_data['bachelor_major']:
                        country = row_data['bachelor_major_country']
                        if not country:
                            country = ''
                        bachelor_data = EducationHistoy.objects.create(
                            major = row_data['bachelor_major'],
                            country=country,
                            education_level='B'
                        )
                        student.education_history.add(bachelor_data)
                    if row_data['master_major']:
                        country = row_data['master_major_country']
                        if not country:
                            country = ''
                        masters_data = EducationHistoy.objects.create(
                            major = row_data['master_major'],
                            country=country,
                            education_level='M'
                        )
                        student.education_history.add(masters_data)
                    if row_data['note']:
                        note_data = StudentNotes.objects.create(
                            note=row_data['note'],
                            created_date=date.today(),
                        ...
                        student.notes.add(note_data)
            return JsonResponse({'upload_status': 'complete'})
        except Exception as e:
            return JsonResponse({'upload_status': 'Oops! something went wrong.'})
